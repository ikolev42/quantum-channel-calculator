...
classical_channels = {
    'Binary Symmetric': {
        ...
        'description': 'Binary Symmetric Channel (BSC) is a classical channel where each bit has probability α to be flipped (0 -> 1 or 1 -> 0). It models noise in classical communications. The capacity C is the maximum information rate, achievable rate R depends on input distribution λ. α is the noise parameter, between 0 and 0.5. Binary entropy H is used for calculation.'
    },
    'Binary Erasure': {
        'param_name': 'α',
        'range': [0, 1],
...
# --- Quantum matrix: 8 fields + λ, 1-λ, ρ0, ρ1, +, ρ ---
...
def update_classical_a(*args):
    try:
        ...
        a_lower_entry.configure(state="normal"); a_lower_entry.delete(0, "end"); a_lower_entry.insert(0, f"{a:.6f}"); a_lower_entry.configure(state="disabled")
    except: pass
...
def generate_example():
    ...
    calculate(); generate_graph(False)
...
def update_mode(val):
    ...
    if mode == 0:
        quantum_matrix_frame.pack_forget()
        classical_matrix_frame.pack(pady=10)
        a_classical_entry.delete(0, "end"); a_classical_entry.insert(0, "0.5"); update_classical_a()
    else:
        classical_matrix_frame.pack_forget()
        quantum_matrix_frame.pack(pady=10)
